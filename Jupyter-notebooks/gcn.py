...
pk.dump(h, open('/ix/djishnu/Hanxi/PGM_Project/GCN_model_embed.pkl', 'wb'))
# %%
...
def generate_sequence(embeddings, adjacency_matrix, start_node, walk_length, restart_prob, top_n=5):
    ...
    while len(walk) < walk_length:
        ...
        if random.random() < restart_prob:
            walk = [start_node]  # Restart the walk
        else:
            cur = walk[-1]
...
for _ in range(num_walks_per_node):
    walk = generate_sequence(h.detach().numpy(), cosine_mat, starting_node, walk_length, restart_prob, top_n=5)
